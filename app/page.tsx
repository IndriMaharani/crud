            ...
            <table className='w-full'>
                ...
                <tbody>
                    {Object.values(getValue).map((data: any, index: number) => (
                        //<div key={index}>
                        //  <div>
                        //    {data.npm} - {data.nama} - {data.jurusan}
                        //</div>
                        //</div>
                        ...
                        <tr key={index}>
                            <td className='text-center border border-gray-700 p-2.5'>
                                {/* icon edit*/}
                                <Link href={`/edit/${btoa(data.npm)}/${atob("MjIzMTIwMjA=")}`} className='bg-emerald-700 text-white px-2.5 py-5px rounded-md mr-5px text-sm' title='edit data'>
                                    <FontAwesomeIcon icon={faPencil}></FontAwesomeIcon>
                                </Link>

                                {/* icon delete*/}
                                <Link href={"/"} className='bg-rose-700 text-white px-2.5 py-5px rounded-md ml-5px text-sm' title='hapus data' onClick={() => {setDelete(data.npm, data.nama)}}>
                                    <FontAwesomeIcon icon={faTrash}></FontAwesomeIcon>
                                </Link>
                            </td>
                            <td className='text-center border border-gray-700'>{data.npm}</td>
                            <td className='text-justify border border-gray-700 px-2.5'>{data.nama}</td>
                            <td className='text-center border border-gray-700'>{data.jurusan}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
